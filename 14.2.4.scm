...
(define (contains-bt-list n bt)
  (cond
    [(equal? false bt) (list false)]
    [else (append 
           (cond 
             [(< n (node-ssn bt) ) (contains-bt-list n (node-left bt))]
             [(> n (node-ssn bt) ) (contains-bt-list n (node-right bt))]
             [else (list (node-name bt))]))]))

(define (contains-bt-list-old n bt)
  (cond
    [(equal? false bt) (list false)]
    [(equal?  n (node-ssn bt)) (list (node-name bt))]
    [else (append (contains-bt-list-old n (node-left bt))
             (contains-bt-list-old n (node-right bt)))]))
